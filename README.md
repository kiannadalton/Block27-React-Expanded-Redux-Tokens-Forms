...
In the React Jest Testing Workshop, I was able to learn the major difference between getByRole and queryByRole. getByRole is useful when expecting an element to be within the document, while queryByRole is better used to test the absence of the element. It was critical for the workshop, as getByRole needs the element to perform the test and it was failing as a result of that. By using queryByRole, it allowed me to run the test of the initial absense of puppy data.

## Demonstrates use of:

React, Redux, Redux Toolkit, Json Web Tokens (JWT), Bearer, onSubmit, onClick, async/await, fetch, useState, useEffect GET, POST, Displaying error messages, spread operator (...), JSON.stringify, .json(), props, if, preventDefault(), useDispatch, configureStore, createSlice, useSelector, nested objects, createApi, fetchBaseQuery, useQuery, useMutation, middleware, .find, destructuring, SCSS, Jest, describe, test, expect, getByLabelText, queryByLabelText, getByRole, queryByRole, getByText, queryByText, toHaveClass, screen, render, fireEvent, click,
